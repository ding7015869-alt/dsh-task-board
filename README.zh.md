...
**新建任务（下半部）**——定时编辑器（每天 / 每周 / 每月 / 间隔 / 自定义，星期芯片 + 精确到分钟）、父子任务多选、所属项目选择，以及工作区 / 模式 / 权限钉子。
...
- **会话行菜单入口**：会话列表里每个会话行的「…」菜单（三个点按钮）注入**「添加为任务卡」**项，**仅当该会话未被未归档任务卡占用**时显示（`taskClaimedSessionIds`：任务的最近一次执行会话、全部保留的执行会话、冻结快照的来源会话、卡片创建时绑定的来源会话——已归档任务释放其会话，项随之重新出现）。点击经模块级请求总线（`new-task-request.ts`）发出新建任务请求，以**会话标题预填标题**打开「新建任务」对话框（只预填标题，描述 / Prompt / 钉住项留空）。新建的任务卡**绑定该来源会话**（任务记录持久化 `sourceSession`）：卡片与详情展示来源会话标记（卡片 meta 行「会话」前缀 chip / 详情「来源会话」section 带「查看会话」跳转按钮）；任务执行时若该会话空闲则**在该会话内继续**（优先于「复用上次执行会话」规则；会话名册未知或该会话不空闲时 fail-closed 走新会话）；来源会话进入占用集合——建卡后该会话的菜单项消失，归档任务后重新出现；导入门不继承该绑定。对话框渲染在 **body 级独立 React 根**（`session-new-task-overlay.tsx`）——点击侧边栏行会把中间列还给会话视图，看板面板里的对话框副本会不可见。注入行是纯 DOM，由与侧边栏入口同款的自修复 MutationObserver 维护；会话身份从行 React fiber 读取（shell 把会话 id 留在 React state 而非 DOM 属性上），读不到 fiber 的行直接跳过、绝不猜测。observer 重扫对稳定行严格 no-op——重写未变化的标签会记录新的 mutation 并喂给自己的回调（该行只在标签值真正变化时才触碰 DOM）。
...
- **卡片会话按钮**：每张卡片右上角有放大镜按钮，直接跳到该任务最近一次执行的会话；还没有执行会话时跳到卡片的来源会话（创建时绑定的那个会话，卡片 meta 行「会话」前缀 chip 即它）；完全没有会话可跳时，回退为打开任务详情。
...
- **定时任务**：新建任务与详情面板均可配置定时执行——启用开关 + 参考流行提醒应用重构的**结构化定时编辑器**：频率选项卡（每天 / 每周 / 每月 / 间隔 / 自定义）、星期芯片选择（工作日 / 周末快速组 + 周一至周日逐日点选）、精确到分钟的 时:分 输入、每月的「日」选择器、间隔步进（快速 5/10/15/30 分钟与 1/2/3/6/12 小时）。每个结构化选项都序列化为同一套 5 段 cron 表达式（分 时 日 月 周，支持 `*` / `*/n` / `a-b` / 逗号列表）；特殊表达式可在「自定义」页直接输入（无效的中间状态保留上一个合法 cron）。启用即计算并持久化「下次运行时间」，卡片显示定时标识；到点自动走真实执行链路（同手动执行），执行会话照常可跳转。
...
tests/*.spec.ts                                    # 303 个自动化测试（账本 / 状态机 / cron / spec 桥 / 门控 / 自动化 / UI 冒烟）
...
### 2) 从本仓 GitHub Release 安装（预构建 tgz，取最新版）
...
全家桶聚合包 `@linxin666/dsh-web-ui-all` 可连同 DSH Web GUI 插件全家桶一起安装本插件。

安装后**重启 `dsh web`**，侧边栏「新会话」下方出现「任务看板」入口即生效；页面刷新不够，需重启进程。

## 构建

前置：Node ≥ 20，官方 NPM SDK 可访问。类型与运行时 API 全部来自官方 NPM SDK（`@deepseek-ai/*` devDependencies），浏览器 bundle 预置在 `build/`，**无需任何 DSH 源码 checkout**。
...
pnpm test           # vitest：303 个测试（账本 / 状态机 / cron / spec 桥 / 门控 / 自动化 / UI 冒烟）
```

`dsh plugin --profile web add link:$(pwd)` 之前必须先跑 `pnpm run build`：profile 通过 `lib/`（`main` / `exports`）解析本包，而 `lib/` 不入库。
...
7. 定时任务：详情 →「定时运行」勾选启用；定时编辑器（每天 / 每周 / 每月 / 间隔 / 自定义 选项卡）切到「间隔」点快速步「10」（cron `*/10 * * * *`），也可点星期芯片 + 精确 时:分；卡片出现定时标识；等待下一个整 10 分钟点，观察卡片自动进入「进行中」并最终完成，详情「上次触发」出现时间、执行记录新增一条（会话可跳转）。
...
10. 会话行菜单：在会话列表打开一个未被任何未归档任务卡引用的会话的「…」菜单 → 出现「添加为任务卡」项 → 点击 → 「新建任务」对话框以该会话标题预填标题打开；被未归档任务卡引用的会话（其最近/保留执行会话、冻结来源会话、绑定来源会话）不显示该项；把占用任务归档后，该项重新出现；项标签随 GUI 语言切换更新。
11. 来源会话绑定：由会话行菜单创建的卡片 meta 行出现「会话 <会话标题>」chip（悬停提示「来源会话」）、详情出现「来源会话」section（带「查看会话」跳转按钮）；卡片放大镜按钮跳向该来源会话（还没有执行会话时）；该会话空闲时执行**在该会话内继续**（优先于复用上次执行会话的规则）；卡片创建后来源会话立即进入占用集合（菜单项消失）。
12. 设置卡：设置页中的任务看板卡显示六个字段（`enabled`、`announceToAgent`、`preventIdleSleep`、`maxConcurrency`、`patrolEnabled`、`patrolIntervalMs`）及其实效值；把 `maxConcurrency` 改成 2 并保存 → 看板头部徽标即时更新，无需重启；打开 `announceToAgent` → 下一个 agent 的系统提示词里出现 `plugin:task-board` 段（关闭后消失）；输入非法值（如 `maxConcurrency` 填 `0`）→ 保存被阻止并标红该字段，写入被拒时显示宿主返回的原因而不是静默丢弃。
13. 刷新页面/重启 DSH → 任务仍在；卸载插件 → GUI 恢复原状。

## 验收对照

- 挂载后侧边栏出现「任务看板」入口；点击切换看板，点会话项返回对话视图
- 会话行菜单：未被未归档任务卡占用（其最近执行会话 / 保留执行会话 / 冻结来源会话 / 绑定来源会话）的会话，「…」菜单出现「添加为任务卡」项；点击在 body 级打开「新建任务」对话框（标题预填为会话标题）；被占用的会话不显示该项，占用任务归档后重新出现
- 来源会话绑定：会话行创建的卡片持久化 `sourceSession`（宿主账本），卡片 meta 行「会话」前缀 chip 与详情「来源会话」section（带「查看会话」跳转按钮）展示来源会话；卡片放大镜按钮在无执行会话时跳向来源会话；该会话空闲时执行在其内继续（优先于「复用上次执行会话」规则，名册未知/不空闲时 fail-closed 走新会话）；来源会话进入占用集合（建卡后菜单项消失）；导入门不继承该绑定
...
- 删除有确认环节，删除后本地存储同步移除
- 定时任务：定时编辑器（每天 / 每周 / 每月 / 间隔 / 自定义 选项卡、星期芯片、精确 时:分、每月日期选择、间隔步进、自定义 cron）配置与校验、下次运行时间、到点自动真实执行、状态回写、定时卡片标识、重启后调度恢复（宿主侧调度——浏览器可关闭；宕机期间错过的触发点跳过而不补跑）
...
- README + 覆盖存储读写/状态流转/执行触发/cron 解析/调度器/依赖门控（宿主门控 + 定时跳过 + 删除 fail-open）/看板自动化（并发上限 + 巡逻派发 + settle 驱动自动续跑 + 全条件判定）/会话行「添加为任务卡」菜单项（占用判定 + 新建任务预填 + 纯 DOM 自修复，无变异反馈循环）/来源会话绑定（持久化 + 占用谓词 + 执行会话选择规则 + 对话框 wire 门 + 卡片/详情展示）的自动化测试
