 ...
 * idle-sleep protection. Registers into the Host settings surface's
 * configurable-plugins slot (`settings.plugin.item`), keyed by the
 * `task-board` settings namespace the Host serves.
 */
...
/** An integer field with a minimum bound: non-integer or below-min drafts are rejected. */
function intField(field: string, min: number): FieldSpec {
  return {
    field,
...
export interface TaskBoardSettings {
  ...
  /** Whether the periodic patrol dispatcher auto-dispatches new todo tasks. */
...
export interface TaskBoardSettingsCardState extends CardShell {
  ...
  /** The board-wide concurrency cap. */
  maxConcurrency: CardFieldState
  /** The patrol dispatcher switch. */
  patrolEnabled: CardFieldState
  /** The patrol scan interval. */
  patrolIntervalMs: CardFieldState
}
...
export class TaskBoardSettingsCardController {
  ...
  constructor(scope: SettingsScope<TaskBoardSettings>) {
    this.form = new CardForm(scope, [
      ...
      intField('maxConcurrency', 1),
      booleanField('patrolEnabled'),
      intField('patrolIntervalMs', 1000),
    ])
  ...
  private projection(): TaskBoardSettingsCardState {
    return {
      ...
      patrolEnabled: this.form.field('patrolEnabled'),
      patrolIntervalMs: this.form.field('patrolIntervalMs'),
    }
  }
...
export function TaskBoardSettingsCard(props: TaskBoardSettingsCardProps) {
  ...
  return (
    ...
    >
      ...
      />
      <ValueField
        id="settings-task-board-max-concurrency"
        label={t('settings.maxConcurrency')}
        hint={t('settings.maxConcurrencyHint')}
        placeholder="1"
        numeric
        {...fieldProps}
      ...
      />
      <BooleanField
        id="settings-task-board-patrol-enabled"
        label={t('settings.patrolEnabled')}
        hint={t('settings.patrolEnabledHint')}
        inheritLabel={t('settings.inherit')}
        onLabel={t('settings.on')}
        offLabel={t('settings.off')}
        {...fieldProps}
        {...state.patrolEnabled}
        onEdit={(text) => { props.edit('patrolEnabled', text) }}
        onReset={() => { props.resetField('patrolEnabled') }}
      />
      <ValueField
        id="settings-task-board-patrol-interval"
        label={t('settings.patrolInterval')}
        hint={t('settings.patrolIntervalHint')}
        placeholder="30000"
        numeric
        {...fieldProps}
        {...state.patrolIntervalMs}
        onEdit={(text) => { props.edit('patrolIntervalMs', text) }}
        onReset={() => { props.resetField('patrolIntervalMs') }}
      />
      <p>
