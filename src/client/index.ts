...
import { mountNewTaskOverlay } from './session-new-task-overlay.tsx'
import { mountSessionTaskCardEntry } from './session-menu-entry.ts'
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  ...
  interface SlotMap {
    /**
     * The child slot the Host settings surface (ui-settings-plugins)
     * dispatches per settings namespace: the configurable-plugins tab
     * renders the intersection of the namespaces the Host serves and the
     * cards registered here. Spelled here with the same keyed shape so
     * this package can register without depending on the sibling UI
     * package.
     */
...
export function apply(ctx: ClientContext): void {
  ...
  // namespace, registered into the Host settings surface's configurable
  // plugins slot, keyed by the namespace the Host serves.
  ...
  const mountUi = (): void => {
    ...
    /** Host 模型目录的最小结构（session/modelCatalog 的 value）。 */
    interface ModelCatalogValue {
    ...
    /**
     * 读 Host 的「已配置模型目录」，与 GUI 模型选择器同一个 remote face：
     * `remote.session.modelCatalog()` 返回 RemoteResult `{ ok, value }`。
     * 上游读的是 `connection.api.sessions.modelCatalog()`，而 connection 服务
     * 没有 `api` 成员，所以模型下拉一直是空的。
     */
    const readModelCatalog = async (): Promise<ModelCatalogValue | undefined> => {
      try {
        const face = (remote as Partial<ClientRemote>).session
        if (face === undefined || typeof face.modelCatalog !== 'function') return undefined
        const response = await face.modelCatalog()
        if (response.ok) return response.value as unknown as ModelCatalogValue
        console.warn('[dsh-task-board] model catalog unavailable: ' + response.error.code + ': ' + response.error.message)
        return undefined
      } catch (error) {
        console.warn('[dsh-task-board] model catalog read failed', error)
        return undefined
      }
    }
    ...
    const pushModelOptions = async (): Promise<void> => {
      try {
        ...
        // remote 命名空间可能晚于本插件挂载，读空时按有界退避重试几次。
        let catalog: ModelCatalogValue | undefined
        for (let attempt = 0; attempt < 3 && catalog === undefined; attempt++) {
          catalog = await readModelCatalog()
          if (catalog === undefined) await new Promise(resolve => setTimeout(resolve, 1200))
        }
        for (const group of catalog?.groups ?? []) {
          for (const model of group.models ?? []) {
            const name = model.name ?? model.id
            // 该模型声明的推理强度档位（目录 reasoning.efforts）；未声明的
            // 路由不带该字段，UI 侧据此决定是否渲染强度下拉。
            const efforts: Array<{ id: string; name?: string; description?: string }> = []
            for (const rawEffort of model.reasoning?.efforts ?? []) {
              const effortId = rawEffort.id
              if (typeof effortId !== 'string' || effortId === '') continue
              efforts.push({
                id: effortId,
                ...(typeof rawEffort.name === 'string' && rawEffort.name !== '' ? { name: rawEffort.name } : {}),
                ...(typeof rawEffort.description === 'string' && rawEffort.description !== '' ? { description: rawEffort.description } : {}),
              })
            }
            models.push({
              id: group.id !== undefined ? `${group.id}/${model.id}` : model.id,
              name,
              label: name,
              ...(group.id === undefined ? {} : { provider: group.id }),
        ...
        if (fallback?.model !== undefined && fallback.model !== '') {
          defaultModel = fallback.provider !== undefined && fallback.provider !== ''
            ? `${fallback.provider}/${fallback.model}`
            : fallback.model
        }
        if (models.length > 0 || defaultModel !== undefined) {
          controller.setExecutionOptions({ models, ...(defaultModel === undefined ? {} : { defaultModel }) })
        }
      } catch (error) {
    ...
    try {
      ...
      // Session-row "add as task card": the standalone modal overlay first
      // (it owns the request bus the menu row publishes to), then the menu
      // injection itself.
      disposers.push(mountNewTaskOverlay(controller, ctx.locale))
      disposers.push(mountSessionTaskCardEntry(controller, ctx.locale))
    } catch (error) {
