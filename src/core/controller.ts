...
import { DEFAULT_PROJECT_ID, type ProjectRecord } from './projects.ts'
...
/** One reasoning-effort level a model route supports (catalog metadata). */
export interface ExecutionReasoningEffort {
  /** Effort id (the value sent to `session.selectModel` as `reasoningEffort`). */
  id: string
  /** Display name for the picker row (falls back to the id). */
  name?: string
  /** Optional description the catalog carries. */
  description?: string
}
...
export interface ExecutionModelOption {
  ...
  name?: string
  label?: string
  /** Provider id; also the picker's grouping key. */
  provider?: string
  /** Provider display name for the group heading (falls back to `provider`). */
  groupName?: string
  /** Reasoning-effort levels the route supports (absent when the adapter declares none). */
  reasoningEfforts?: readonly ExecutionReasoningEffort[]
  /** The route's default effort id (absent when the provider has no default). */
...
export interface ExecutionOptionsSnapshot {
  ...
  /** The deployment default route (`provider/model`) from the Host model catalog. */
  defaultModel?: string
}
...
export interface ControllerSnapshot {
  ...
  /** The board's projects (Host ledger v4; empty on the legacy fallback, where the project UI stays hidden). */
  projects?: readonly ProjectRecord[]
  /** The fixed default-project id; every unowned card lands there. */
  defaultProjectId?: string
  /** The project the board is filtered to; undefined = every project. */
  selectedProjectId?: string
  transportError?: string
  host?: Pick<TaskBoardSnapshot, 'revision' | 'scheduler' | 'power' | 'sessionDefaultPermission' | 'automation' | 'nextSerial'>
}
...
export class BoardController {
  ...
  private defaultProjectId: string = DEFAULT_PROJECT_ID
  ...
  getSnapshot(): ControllerSnapshot {
    return {
      ...
      ...(this.selectedProjectId === undefined ? {} : { selectedProjectId: this.selectedProjectId }),
  ...
  // --- project mutations (Host ledger v4; presentation-level grouping) ---------

  /** Filter the board to one project (undefined = every project). Unknown ids are ignored. */
  setSelectedProject(id: string | undefined): void {
    if (id !== undefined && !this.projects.some(project => project.id === id)) return
    this.selectedProjectId = id
    this.notify()
  }

  /**
   * Create a project on the Host ledger (client-minted id, bounded name,
   * auto-assigned palette color). Legacy fallback (no transport) has no
   * project layer: the project UI stays hidden and the call yields nothing.
   * @returns the created project once the Host has confirmed it.
   */
  async createProject(name: string, color?: string): Promise<ProjectRecord | undefined> {
    if (this.deps.transport === undefined) return undefined
    const id = this.uuid()
    const accepted = await this.commitRemote({ kind: 'create-project', id, name, ...(color === undefined ? {} : { color }) }, id)
  ...
  async renameProject(id: string, name: string): Promise<boolean> {
    ...
    return await this.commitRemote({ kind: 'rename-project', projectId: id, name }, id)
  }

  /** Recolor a project to one of the eight palette slots. */
  async setProjectColor(id: string, color: string): Promise<boolean> {
    if (this.deps.transport === undefined) return false
    return await this.commitRemote({ kind: 'set-project-color', projectId: id, color }, id)
  }

  /**
   * Delete a project on the Host: its cards are reassigned to the default
   * project in the same commit (serials and execution history untouched).
   * The default project itself is reserved by the ledger and never accepted.
   */
  async deleteProject(id: string): Promise<boolean> {
    if (this.deps.transport === undefined) return false
    const accepted = await this.commitRemote({ kind: 'delete-project', projectId: id }, id)
    if (accepted && this.selectedProjectId === id) {
  ...
  deleteTask(id: string): void {
    ...
    const { tasks, selectionCleared } = applyDeleteTask(this.tasks, this.selectedTaskId, id, this.now())
    this.tasks = [...tasks]
  ...
  /**
   * Reject a settled card (done/failed) with modification requirements:
   * the Host ledger appends the note to the card's rework history and opens
   * a rework execution that continues the last execution session (or a
   * fresh one when it is gone). At the concurrency cap the card parks in
   * the todo column and the note stays unconsumed until a slot frees.
  ...
  private onRemoteEvent(event: TaskBoardEventPayload | undefined): void {
    if (event !== undefined && this.hostState !== undefined && event.revision === this.hostState.revision
      ...
      this.hostState = { revision: event.revision, scheduler: event.scheduler, power: event.power, ...(event.automation !== undefined ? { automation: event.automation } : {}) }
      this.notify()
      return
    }
  ...
  private acceptRemote(snapshot: TaskBoardSnapshot): boolean {
    ...
    this.projects = snapshot.projects === undefined ? [] : [...snapshot.projects]
    if (typeof snapshot.defaultProjectId === 'string' && snapshot.defaultProjectId !== '') this.defaultProjectId = snapshot.defaultProjectId
    this.hostState = { revision: snapshot.revision, scheduler: snapshot.scheduler, power: snapshot.power, ...(snapshot.automation !== undefined ? { automation: snapshot.automation } : {}) }
    this.transportError = undefined
    if (this.selectedTaskId !== undefined && !this.tasks.some(task => task.id === this.selectedTaskId)) {
      this.selectedTaskId = undefined
    }
    // A board filtered to a project that no longer exists (deleted on the
    // Host, or a pre-v4 Host that carries no projects) falls back to every
    // project rather than filtering the board down to nothing.
    if (this.selectedProjectId !== undefined && !this.projects.some(project => project.id === this.selectedProjectId)) {
      this.selectedProjectId = undefined
    }
    if (!this.archiveView && this.selectedTaskId !== undefined
