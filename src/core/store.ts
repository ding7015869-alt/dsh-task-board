...
function isTaskRecordShape(value: unknown): value is Omit<TaskRecord, 'status'> & { status: unknown } {
  ...
  if (record.reasoningEffort !== undefined && typeof record.reasoningEffort !== 'string') return false
  ...
  if (record.parentIds !== undefined && (!Array.isArray(record.parentIds) || record.parentIds.some(value => typeof value !== 'string'))) return false
  if (record.serial !== undefined && (typeof record.serial !== 'number' || !Number.isFinite(record.serial))) return false
  // Rework/reject history: only the array-ness is shape-checked; individual
  // entries are REPAIRED per item by normalizeReworkNotes (mirrors the
  // schedule policy — a bad entry must not drop the whole task row). The
  // wire path (the import gate) applies the strict entry-level check.
  if (record.reworkNotes !== undefined && !Array.isArray(record.reworkNotes)) return false
  ...
  for (const execution of record.executions) {
    ...
    if (entry.rework !== undefined && typeof entry.rework !== 'boolean') return false
  }
  return true
}
...
/**
 * Repair a persisted rework/reject history: keep the valid notes in order
 * (oldest first), drop the invalid entries on their own, and yield undefined
 * when nothing usable remains (mirrors the schedule/handover repair policy).
 */
function normalizeReworkNotes(value: unknown): ReworkNote[] | undefined {
  ...
  for (const item of value) {
    ...
    if (typeof note.note !== 'string' || note.note === '') continue
    if (typeof note.at !== 'number' || !Number.isFinite(note.at)) continue
    const consumedAt = typeof note.consumedAt === 'number' && Number.isFinite(note.consumedAt) ? note.consumedAt : undefined
    notes.push({ note: note.note, at: note.at, ...(consumedAt === undefined ? {} : { consumedAt }) })
  }
...
export function parseLedger(raw: string | null): TaskRecord[] {
  ...
  for (const row of parsed) {
    ...
    // The pinned reasoning effort normalizes like its model twin: blank
    // strings clear the pin, so an old UI can never persist an empty effort.
    task.reasoningEffort = normalizeTargetId(row.reasoningEffort)
    ...
    task.parentIds = normalizeParentIds(row.parentIds as readonly string[] | undefined, row.id)
    ...
    // The card serial normalizes like the other optional stamps: a valid
    // positive integer is kept, anything else clears the field (the Host
    // ledger backfills missing numbers at load; the browser store just
    // preserves what it finds).
    task.serial = typeof row.serial === 'number' && Number.isSafeInteger(row.serial) && row.serial >= 1 ? row.serial : undefined
    // The owning project (ledger v4) normalizes like an execution target:
    // a blank string clears the pin; the Host ledger lands unowned cards in
    // the default project at load time.
    task.projectId = normalizeTargetId(row.projectId as string | undefined)
    // Rework/reject history repairs like the other optional arrays: valid
    // notes are kept in order (oldest first), bad entries drop on their own.
    task.reworkNotes = normalizeReworkNotes(row.reworkNotes)
    // An execution's rework flag is meaningful only when exactly true:
    // clear any false/other value so the launcher sees a clean record.
    task.executions = task.executions.map(entry => {
      const record = entry as unknown as Record<string, unknown>
      if (record.rework === undefined || record.rework === true) return entry
      const { rework: _rework, ...rest } = record
      return rest as unknown as ExecutionRecord
    })
    tasks.push(task)
  }
  return tasks
