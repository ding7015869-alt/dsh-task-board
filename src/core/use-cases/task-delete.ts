 ...
 * Removing a task also drops it from every other task's parentIds: a deleted
 * parent must not wedge its children (the gate ignores missing parents), so
 * the dangling ids are stripped in the same transition.
 ...
 * @param now - the ledger transition instant (bumped on touched rows).
 */
export function applyDeleteTask(
  ...
  const next = tasks
    ...
    .map(task => task.parentIds.includes(id)
      ? { ...task, parentIds: task.parentIds.filter(parent => parent !== id), updatedAt: now }
      : task)
  return {
    tasks: next,
