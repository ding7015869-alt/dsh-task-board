 ...
 */
import { freezeOf, isTaskPermission, normalizeParentIds, normalizeTargetId, type TaskRecord, type TaskPermission } from '../tasks.ts'
 ...
 */
export type TaskUpdatePatch = Partial<Pick<TaskRecord, 'title' | 'description' | 'prompt' | 'workspaceId' | 'mode' | 'permission' | 'model' | 'reasoningEffort' | 'reuseSession' | 'projectId'>> & {
  ...
  /**
   * Replaces the task's parent task ids (dependency gating); an explicit null
   * clears the gate. Children are the derived reverse of this field, never
   * stored on the task.
   */
  parentIds?: string[] | null
  /**
   * Replaces the task's CHILD set: the task ids that shall list this task
   * among their parents (multiple allowed). The transition maintains both
   * directions atomically on the child tasks' own parentIds; an explicit
   * null clears the set.
   */
...
export function applyUpdateTask(
  ...
  // The desired child set (the inverse edge) is maintained on the child
  // tasks' own parentIds, so it is computed up front and applied in the same
  // single pass: tasks that must gain/lose the edge get a fresh row, every
  // other row keeps its reference.
  let desiredChildren: ReadonlySet<string> | undefined
  if ('childIds' in patch) {
    desiredChildren = new Set(normalizeParentIds(patch.childIds ?? [], id))
  }
  let newParents: string[] | undefined
  if ('parentIds' in patch) {
    newParents = normalizeParentIds(patch.parentIds ?? [], id)
  }
  return tasks.map(task => {
    if (task.id !== id) {
      if (desiredChildren === undefined) return task
      const hasEdge = task.parentIds.includes(id)
      const wantsEdge = desiredChildren.has(task.id)
      if (hasEdge === wantsEdge) return task
      return {
        ...task,
        parentIds: wantsEdge ? [...task.parentIds, id] : task.parentIds.filter(parent => parent !== id),
        updatedAt: now,
      }
    }
    const { freeze: freezePatch, handover: handoverPatch, parentIds: _parentIds, childIds: _childIds, ...rest } = patch
    ...
    const reasoningEffort = 'reasoningEffort' in patch ? normalizeTargetId(patch.reasoningEffort) : undefined
    ...
    if (reasoningEffort !== undefined || 'reasoningEffort' in patch) next.reasoningEffort = reasoningEffort
    // The owning project (ledger v4) normalizes like an execution target:
    // a blank string clears the pin; the Host ledger lands an unowned card
    // in the default project at commit time.
    if ('projectId' in patch) next.projectId = normalizeTargetId(patch.projectId)
    if (newParents !== undefined) next.parentIds = newParents
    return next
  })
}
