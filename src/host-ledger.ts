...
import { canMoveManually, markConsumedReworkNotes, pendingParentTasks, retainRecentExecutions, settleExecution, startExecution, withStatus, type ExecutionRecord, type TaskRecord } from './core/tasks.ts'
...
import { TASK_BOARD_LEGACY_SCHEMA_VERSION, TASK_BOARD_PREV_SCHEMA_VERSION, TASK_BOARD_SCHEMA_VERSION, type TaskBoardAction, type TaskBoardSchedulerSnapshot } from './protocol.ts'
import { DEFAULT_SESSION_PERMISSION, requiresPermissionConfirmation, type TaskPermission } from './core/handover.ts'
import { checkExecutable, openExecutionCount, CONTINUE_INITIATOR, PATROL_INITIATOR, PATROL_STATUSES } from './core/executability.ts'
import { DEFAULT_PROJECT_ID, defaultProject, normalizeProjectList, projectOf, type ProjectRecord } from './core/projects.ts'
import { applyCreateProject, applyDeleteProject, applyRenameProject, applySetProjectColor } from './core/use-cases/project.ts'
...
interface LedgerDocument {
  ...
  revision: number
  tasks: TaskRecord[]
  /** The board's projects (v4); the default project is always present. */
  projects: ProjectRecord[]
  scheduler: PersistedScheduler
  recentRequests: PersistedRequest[]
  /** Next card serial to mint; equals the highest serial ever issued. */
  nextSerial: number
}
...
export interface LedgerState {
  revision: number
  tasks: TaskRecord[]
  /** The board's projects (the default project always first by ledger order). */
  projects: ProjectRecord[]
  scheduler: TaskBoardSchedulerSnapshot
  /** Next card serial the Host will mint on the next create. */
  nextSerial: number
}

/**
 * Fill the card-serial gaps in one pass, in the stable "createdAt then id"
 * order: every serial-less task receives the next unused number starting at
 * max(1, startNext), valid serials already on rows are kept as-is, and the
 * counter only ever moves forward — deleted tasks' numbers are never
 * reissued. Returns the counter to persist as nextSerial.
 */
export function assignMissingSerials(tasks: TaskRecord[], startNext: number): number {
  const ordered = [...tasks].sort((a, b) => a.createdAt - b.createdAt || a.id.localeCompare(b.id))
  // Pass 1: keep valid serials; the first row of a duplicated number keeps it,
  // every other occurrence (and every invalid one) is cleared for re-minting,
  // so two cards can never end up wearing the same number.
  const kept = new Set<number>()
  for (const task of ordered) {
    const s = task.serial as number | undefined
    if (typeof s === 'number' && Number.isSafeInteger(s) && s >= 1 && !kept.has(s)) kept.add(s)
    else task.serial = undefined
  }
  // The mint point only moves forward: it starts at the persisted counter and
  // is pushed past every kept number, so a deleted task's number is never
  // handed out again.
  let next = Math.max(1, Number.isSafeInteger(startNext) ? startNext : 1)
  for (const s of kept) if (s >= next) next = s + 1
  for (const task of ordered) {
    const s = task.serial as number | undefined
    if (typeof s === 'number' && Number.isSafeInteger(s) && s >= 1) continue
    task.serial = next
    next += 1
  }
  return next
...
export interface OpenExecutionReference {
  ...
  /** True when this execution carries the card's pending rework notes into its prompt. */
...
/** Clamp a configured concurrency cap to a usable integer (>= 1); bad values fall back to 1. */
...
export class HostTaskLedger {
  ...
  /** Board-wide concurrency cap; live-adjustable through setMaxConcurrency. */
  private maxConcurrencyValue: number

  constructor(dir: string = join(dshHome(), 'task-board'), private readonly now: () => number = Date.now, options: { sessionDefaultPermission?: TaskPermission; maxConcurrency?: number } = {}) {
    this.sessionDefaultPermission = options.sessionDefaultPermission ?? DEFAULT_SESSION_PERMISSION
    this.maxConcurrencyValue = normalizeConcurrency(options.maxConcurrency)
  ...
  state(): LedgerState {
    ...
    return { revision, tasks: cloneTasks(this.document.tasks), projects: this.document.projects.map(project => ({ ...project })), scheduler, nextSerial: this.document.nextSerial }
  }
  ...
  /** The board-wide concurrency cap: how many tasks may execute at once. */
  get maxConcurrency(): number {
    return this.maxConcurrencyValue
  }

  /**
   * Update the cap live (settings change): open executions keep running, only
   * the ceiling the next dispatcher sees moves.
   */
  setMaxConcurrency(value: number): void {
    this.maxConcurrencyValue = normalizeConcurrency(value)
  }

  /** Board-wide open-execution count: tasks carrying an unsettled execution. */
  openExecutionsCount(): number {
    return openExecutionCount(this.document.tasks)
  }
  ...
  /**
   * Open a due cron occurrence. Task-owned gates (permission confirmation,
   * pending parents, already running) skip the occurrence: they advance
   * nextRunAt and move on. The board-wide concurrency cap instead DEFERS:
   * nothing is committed, so nextRunAt stays put and the next tick retries
   * the same trigger rather than dropping it. Returns the opened run, or
   * undefined when skipped or deferred.
   */
  openScheduled(taskId: string, nextRunAt: number | undefined, triggeredAt: number): OpenedRun | undefined {
    ...
    if (pendingParentTasks(task, this.document.tasks).length > 0) {
      // A pending parent task must never be skipped by cron: the gate skips
      // the occurrence and rolls to the next one, retrying whenever the
      // parents settle to done.
      this.document.tasks = [...applyScheduleNextRun(this.document.tasks, taskId, nextRunAt, task.schedule?.lastTriggeredAt, triggeredAt)]
      this.commit()
      return undefined
    }
    ...
    if (openExecutionCount(this.document.tasks) >= this.maxConcurrencyValue) {
      // Board at its concurrency cap: keep this trigger pending (no commit)
      // so the next tick retries it; the occurrence is not dropped.
      return undefined
    }
  ...
  /**
   * The patrol dispatcher's entry point: open an execution for a task only
   * when EVERY executability condition holds at open time — not archived, a
   * patrol status, no open execution, no active cron (scheduled cards are
   * the scheduler's territory, never the patrol's), all parents settled,
   * permission confirmed, and a concurrency slot free. Returns the opened
   * run, or undefined when any condition blocks.
   */
  openPatrolRun(taskId: string, now: number): OpenedRun | undefined {
    return this.openGatedRun(taskId, now, PATROL_INITIATOR)
  }

  /**
   * The auto-continuation dispatcher's entry point: the settle-driven
   * counterpart of {@link openPatrolRun} — the same full gate, but the opened
   * run is stamped as the continuation of a settled parent, not as a patrol
   * pass. Returns the opened run, or undefined when any condition blocks.
   */
  openContinueRun(taskId: string, now: number): OpenedRun | undefined {
    return this.openGatedRun(taskId, now, CONTINUE_INITIATOR)
  }

  /**
   * The shared gated open behind the patrol and the auto-continuation: open
   * an execution for a task only when EVERY executability condition holds at
   * open time — not archived, a patrol status, no open execution, no active
   * cron (scheduled cards are the scheduler's territory, never a
   * dispatcher's), all parents settled, permission confirmed, and a
   * concurrency slot free.
   */
  private openGatedRun(taskId: string, now: number, initiator: string): OpenedRun | undefined {
    const task = this.document.tasks.find(item => item.id === taskId)
    if (task === undefined) return undefined
    if (task.schedule !== undefined && task.schedule.enabled === true) return undefined
    const check = checkExecutable(task, PATROL_STATUSES, {
      ...
      openExecutions: openExecutionCount(this.document.tasks),
      maxConcurrency: this.maxConcurrencyValue,
    })
    if (!check.executable) return undefined
    const opened = startExecution(task, now, crypto.randomUUID(), initiator)
    this.document.tasks = this.document.tasks.map(item => item.id === task.id ? opened.task : item)
    this.commit()
    return opened
  }
  ...
  /**
   * Mark the card's unconsumed rework notes as consumed — called by the
   * service ONLY after the rework execution queued its prompt into a
   * session. A launch that fails before the prompt is queued leaves the
   * notes unconsumed, so the next execution re-queues them (no lost round).
   */
  consumeRework(taskId: string, executionId: string): void {
    const now = this.now()
    let changed = false
    this.document.tasks = this.document.tasks.map(task => {
      if (task.id !== taskId || task.reworkNotes === undefined) return task
      const execution = task.executions.find(entry => entry.id === executionId)
      if (execution?.rework !== true) return task
      if (!task.reworkNotes.some(note => note.consumedAt === undefined)) return task
      changed = true
      return markConsumedReworkNotes(task, now)
    })
    if (changed) this.commit()
  }
  ...
  private apply(action: TaskBoardAction, initiator?: string): { state: LedgerState; run?: OpenedRun } {
    ...
    switch (action.kind) {
      case 'import': {
        ...
        // Imported rows arrive without card numbers (or with numbers minted by
        // their source ledger): fill the gaps from our own counter so the
        // target's numbering stays forward-only.
        this.document.nextSerial = assignMissingSerials(this.document.tasks, this.document.nextSerial)
        // The owning project (v4): a pin that dangles on THIS board (or an
        // absent one) is backfilled to the default project — grouping only.
        this.document.tasks = this.document.tasks.map(task => ({ ...task, projectId: projectOf(task, this.document.projects) }))
      ...
      case 'create': {
        ...
        // The card serial is minted exactly once, at create: a global counter
        // that is never re-issued after a task is deleted.
        result.task.serial = this.document.nextSerial
        this.document.nextSerial += 1
        // The owning project (v4): a missing or dangling pin lands the card in
        // the default project — grouping only, the freshly minted serial is
        // never touched.
        result.task.projectId = projectOf(result.task, this.document.projects)
      ...
      case 'delete':
        ...
        this.document.tasks = [...applyDeleteTask(this.document.tasks, undefined, action.taskId, now).tasks]
        break
      case 'move': {
      ...
      case 'run': {
        ...
        // Dependency gate: every EXISTING parent task must have settled to
        // `done` before this task may open an execution (manual run or
        // rerun); a parent that was deleted can never wedge its children.
        const pending = pendingParentTasks(task, this.document.tasks)
        if (pending.length > 0) {
          throw new Error(`dependency-gate: ${pending.length} parent task(s) have not settled to done yet: ${pending.map(item => item.title).join(', ')}`)
        }
        // Concurrency gate: the board caps how many tasks may execute at
        // once; at the cap a manual run is refused until a slot frees.
        const openNow = openExecutionCount(this.document.tasks)
        if (openNow >= this.maxConcurrencyValue) {
          throw new Error(`concurrency-gate: the task board is at its concurrency cap (${openNow}/${this.maxConcurrencyValue}); retry once a running execution settles`)
        }
      ...
      case 'rework': {
        // Reject-and-fix flow: a settled card (done/failed) is sent back with
        // modification requirements. The note lands in the card's rework
        // history and the fix execution continues the last execution session
        // (the launcher picks the session; see HostExecutionRunner.launch).
        const task = this.document.tasks.find(item => item.id === action.taskId)
        if (task?.archivedAt !== undefined) throw new Error('archived task is read-only')
        if (task === undefined || task.status === 'running' || hasOpenExecution(task)) throw new Error('task is already running or missing')
        // Rework only applies to settled cards: the rejection refers to a
        // finished attempt, so a card that never ran cannot be reworked.
        if (task.status !== 'done' && task.status !== 'failed') throw new Error('task is not in a settled state')
        if (requiresPermissionConfirmation(task, this.sessionDefaultPermission)) {
          throw new Error(`confirmation-required: the effective permission is above the session default (${this.sessionDefaultPermission}); confirm the card's permission binding first`)
        }
        // Same dependency gate as run/rerun: a parent that has not settled to
        // done blocks the rework dispatch (a failed parent does not unblock).
        const pending = pendingParentTasks(task, this.document.tasks)
        if (pending.length > 0) {
          throw new Error(`dependency-gate: ${pending.length} parent task(s) have not settled to done yet: ${pending.map(item => item.title).join(', ')}`)
        }
        // Append the modification requirement to the card's rework history.
        const withNote: TaskRecord = { ...task, reworkNotes: [...(task.reworkNotes ?? []), { note: action.note, at: now }], updatedAt: now }
        // At the concurrency cap the rework parks in the todo column: the
        // note stays unconsumed until a slot frees, then the next dispatcher
        // (manual run, patrol, or a later rework) carries it into its prompt.
        const openNow = openExecutionCount(this.document.tasks)
        if (openNow >= this.maxConcurrencyValue) {
          this.document.tasks = this.document.tasks.map(item => item.id === task.id ? withStatus(withNote, 'todo', now) : item)
          break
        }
        run = startExecution(withNote, now, crypto.randomUUID(), initiator)
        this.document.tasks = this.document.tasks.map(item => item.id === task.id ? run!.task : item)
        break
      }
      // Project actions (v4): presentation-level grouping of the cards.
      // They never touch card serials, execution history, or any dispatch
      // gate; deleting a project reassigns its cards to the default project.
      case 'create-project':
        this.document.projects = [...applyCreateProject(this.document.projects, action.id, action.name, action.color, now)]
        break
      case 'rename-project':
        this.document.projects = [...applyRenameProject(this.document.projects, action.projectId, action.name, now)]
        break
      case 'set-project-color':
        this.document.projects = [...applySetProjectColor(this.document.projects, action.projectId, action.color, now)]
        break
      case 'delete-project': {
        const result = applyDeleteProject(this.document.projects, this.document.tasks, action.projectId, now)
        this.document.projects = [...result.projects]
        this.document.tasks = [...result.tasks]
        break
      }
    }
  ...
  /**
   * Field-preserving v2 to v4 migration (v3 was serial-only and shared the
   * same shape). It reuses the v4 normalization, but first proves every task
  ...
  private load(dir: string): LedgerDocument {
    ...
    if (parsed.schemaVersion === TASK_BOARD_LEGACY_SCHEMA_VERSION) {
      try {
        ...
        throw new Error(`ledger v2 to v4 migration failed; original file kept at ${this.file}: ${error instanceof Error ? error.message : String(error)}`)
      }
    }
    if (parsed.schemaVersion === TASK_BOARD_PREV_SCHEMA_VERSION) {
      // One-time v3 to v4 migration: seed the project list (default project)
      // and stamp the new generation. Task rows, serials, and the nextSerial
      // counter are carried over untouched by the shared normalization.
      try {
        return this.normalizeDocument({ ...parsed, schemaVersion: TASK_BOARD_SCHEMA_VERSION })
      } catch (error) {
        return this.recoverCorrupt(dir, existed, error)
      }
    }
  ...
  private normalizeDocument(parsed: ParsedLedgerDocument): LedgerDocument {
    ...
    // The project list (v4): repair malformed rows, dedupe, and guarantee the
    // default project is present (older documents arrive without the key).
    const projects = normalizeProjectList(parsed.projects, this.now())
    // The owning project (v4): unowned cards — missing pins on upgraded
    // documents or dangling references from a deleted project — are landed
    // in the default project. Grouping only: serials and execution history
    // are carried over untouched.
    const ownedTasks = tasks.map(task => ({ ...task, projectId: projectOf(task, projects) }))
    // Card serials: an older document has no counter (or a bad one); start
    // from its persisted value when present and mint the gaps so every card
    // on an upgraded ledger carries a stable "#N" in creation order.
    const persistedNextSerial = typeof parsed.nextSerial === 'number' ? parsed.nextSerial : 1
    const nextSerial = assignMissingSerials(ownedTasks, persistedNextSerial)
  ...
  private recoverCorrupt(dir: string, existed: boolean, error: unknown): LedgerDocument {
    ...
    return {
      ...
      revision: 0,
      tasks: [],
      projects: [defaultProject(this.now())],
      nextSerial: 1,
