...
export class HostExecutionRunner {
   ...
   * `options.promptOverride` replaces the composed task prompt for THIS launch
   * only (the rework/reject flow queues the card's modification notes instead
   * of the full prompt, or the full prompt plus the notes on a fresh session);
   * the task record itself is never mutated.
   * @param task - the task to run.
   * @param options - optional session to continue in and one-shot prompt text.
   * @returns the session id the execution runs in.
   */
  async launch(task: TaskRecord, options: { reuseSessionId?: string; promptOverride?: string } = {}): Promise<string> {
    ...
    if (reused !== undefined) {
      try {
        await this.pinAndPrompt(reused, task, permission, options.promptOverride)
      } catch (error) {
    ...
    try {
      ...
      await this.pinAndPrompt(sessionId, task, permission, options.promptOverride)
    } catch (error) {
   ...
   * the same permission/model pins before the prompt. `text` (one-shot)
   * replaces the composed task prompt — the rework flow queues the card's
   * modification notes there; without it the standard composed prompt goes.
   */
  private async pinAndPrompt(sessionId: ExecutionSessionId, task: TaskRecord, permission: TaskPermission | undefined, text?: string): Promise<void> {
    ...
    if (task.model !== undefined && task.model.trim() !== '') {
      ...
      // The pinned reasoning effort rides the same select-model call: the
      // contract validates the effort against the requested model route, and a
      // stale/unknown id degrades through the existing fallback below.
      ...
      try {
        ...
        console.warn(`[dsh-task-board] failed to select model "${task.model}"${effort !== undefined && effort !== '' ? ` at effort "${effort}"` : ''} for session ${sessionId}, falling back to default:`, modelError)
      }
    }
    await this.invoke('session', 'prompt', {
      ...
      mode: 'queue' as const,
      content: [{ type: 'text' as const, text: text ?? promptText(task) }],
    })
  }
