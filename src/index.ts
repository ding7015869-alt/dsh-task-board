...
export const TASK_BOARD_GUIDANCE = '本机已安装 dsh-task-board 插件（DSH Web GUI 的任务看板）：侧边栏「任务看板」入口；在 dsh-web 插件全家桶仓库（packages/dsh-task-board）统一维护，经聚合包 web-ui-all 一键安装。能力：多列看板管理任务；Host 权威账本；关闭浏览器后仍由 Host 执行和结算；任务可钉住工作区、agent 预设和权限；支持 Host 本地时区的 5 段 cron，错过的触发点不补跑；支持单次任务看板并发数量上限与定时巡逻自动派发（巡逻默认关闭，且只在满足全部判定条件时执行）；可选且默认关闭的空闲系统睡眠保护允许屏幕熄灭，但不承诺拦截合盖、手动睡眠、休眠、关机或唤醒已睡眠机器。执行消耗 API 额度。用户提到「任务看板 / 看板 / 定时任务」时即指本插件，请据此协作。若你同时用 todo_write 维护会话顶部的可见计划列表，最终回复前必须再次调用 todo_write 收尾：没有剩余工作时不要保留 in_progress，已完成的最后一步要标为 completed。'
...
export interface Config {
   ...
   * Manual, scheduled, and patrol runs share the cap; at the cap manual runs
   * are refused and due cron triggers stay pending until a slot frees.
   */
  maxConcurrency?: number
  /**
   * The periodic patrol: when on, the Host scans for new todo tasks that
   * satisfy every executability condition (not archived, in the todo pool,
   * not running, no cron ownership, parents done, permission confirmed, a
   * free slot) and dispatches them within the concurrency cap. Scheduled
   * tasks stay with cron. Off by default.
   */
  patrolEnabled?: boolean
  /** The patrol scan interval in milliseconds (>= 1000, default 30000). */
...
export const Config: z<Config> = z.object({
  ...
  patrolEnabled: z.boolean().default(false),
  patrolIntervalMs: z.number().step(1).min(1000).default(30_000),
})
...
function applyImpl(ctx: Context, config?: Config): void {
  const host = new TaskBoardHostService(ctx.typertGateway, {
    ...
    patrolEnabled: config?.patrolEnabled,
    patrolIntervalMs: config?.patrolIntervalMs,
    commandDispatcher: {
  ...
  const sync = (): void => {
    ...
    host.setConfiguration(active, current().preventIdleSleep ?? false, {
      ...
      patrolEnabled: current().patrolEnabled,
      patrolIntervalMs: current().patrolIntervalMs,
    })
    if (!active) return
