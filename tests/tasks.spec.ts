...
import {
  canMoveManually, createTask, executionLabel, latestSessionIdOf, settleExecution, startExecution, withSchedule, withStatus,
...
describe('status transitions', () => {
  it('manual moves are allowed to the parking pools and manual completion', () => {
    ...
    expect(canMoveManually('todo', 'done')).toBe(true)
    expect(canMoveManually('failed', 'done')).toBe(true)
    ...
    expect(canMoveManually('todo', 'failed')).toBe(false)
  })
...
describe('latestSessionIdOf', () => {
  it('returns undefined without executions', () => {
    expect(latestSessionIdOf(sampleTask())).toBeUndefined()
  })

  it('returns the newest execution with a resolved session', () => {
    let task = sampleTask()
    const first = startExecution(task, NOW, 'exec-1')
    task = { ...first.task, executions: [{ ...first.execution, sessionId: 'sess-old' }] }
    // The newest run has not resolved a session yet: fall back to the older one.
    const second = startExecution(task, NOW + 1, 'exec-2')
    expect(latestSessionIdOf(second.task)).toBe('sess-old')
    const settled = { ...second.execution, sessionId: 'sess-new' }
    const withNew = { ...second.task, executions: [...second.task.executions.slice(0, -1), settled] }
    expect(latestSessionIdOf(withNew)).toBe('sess-new')
  })
})
